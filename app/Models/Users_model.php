...
class Users_model extends Crud_model {
    ...
    function authenticate($email, $password) {
        ...
        $this->db_builder->select("id,user_type,client_id,password,first_name,last_name,user_type,is_admin,email");
    ...
    function userApiauthenticate($email, $password) {
        ...
        if ($result_count === 1) {
            $user_info = $result->getRow();
             $checkpass = $this->verify_password($user_info, $password);
             if($checkpass){
               return  $user_info = $result->getRow();
             }else{
                return 'passnotfound'; 
             }
        }else{
